...
export class AppComponent implements OnInit{
  ...
  // C:\Users\Asus\Desktop\get_users_data-master (2)\get_users_data-master\my-angular-app\src\app\app.module.ts
  ngOnInit() {
    ...
    next: (response) => {
      if (response.content && Array.isArray(response.content)) {
        ...
        startPointFeature.setStyle(new Style({
          ...
          zIndex: 10 // Set z-index higher for the start point
        }));
...
private isArrowIcon(style: Style): boolean {
  ...
  return image instanceof Icon && image.getSrc() === 'assets/x.png';
}
close(){
...
fetchData(page: number, rows: number, deviceId?: string, startDate?: string, endDate?: string): void {
  ...
  if (endDate) {
    ...
    // Fetch total distance and time if deviceId, startDate, and endDate are provided
  ...
  this.http.get<any>(url).subscribe(
    response => {
     ...
     const filteredUsers = response.content;
        // this.fetchTotalDistance(response.deviceId,encodeURIComponent(startDate),)
        // console.log(response);
        this.originalUsers = filteredUsers.map((user: any) => {
          this.totalDist += parseFloat(user.delta_distance) || 0;
  ...
  fetchDeviceIdCounts(): void {
    this.http.get<any[]>(this.apiUrl).subscribe(
      response => {
        ...
        // this.filteredDeviceIdCounts = this.deviceIdCounts; // Initialize filtered list
      },
